resource "local_file" "hosts_templatefile" {
  ...
  { name = "databases", instances =  [ for _, vm in yandex_compute_instance.db : vm] },
  { name = "storages", instances = [yandex_compute_instance.storage] }
    ]
  }
)
...
instances_yaml= concat(yandex_compute_instance.web,
		    [for _, vm in yandex_compute_instance.db : vm],
